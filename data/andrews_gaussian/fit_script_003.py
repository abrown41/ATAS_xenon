...
def get_roi(energy_axis,erange=(54.15,58.45)):
...
def fit_lineshapes(energy_axis, *params):
    ...
    *params : list of fit parameters, size: 3*N + 1 where N is the number of lines
    ...
    phi = params[1:-1:3]
    gamma = params[2:-1:3]
#    for e, energy in enumerate(e_res).reverse():
...
lower_bounds = [1e-6,  -2*np.pi, 0.99*gamma_xe1]*3 + [-15]
upper_bounds = [np.inf, 2*np.pi, 1.01*gamma_xe1]*3 + [20]
...
freq_marginal = np.loadtxt("inp.txt")[:,1]

plt.plot(photonenergy,freq_marginal[roi])

p_init, pcov = curve_fit(fit_lineshapes, photonenergy, freq_marginal[roi], p_initial, maxfev=500000)

plt.plot(photonenergy,fit_lineshapes(photonenergy,*p_init),'r')
plt.show()
